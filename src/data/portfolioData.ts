import profileImg from "../assets/4357.jpg";

export const portfolioData = {
  hero: {
    status: "Available for New Opportunities",
    firstName: "Akeburut",
    lastName: "Preemongkon",
    description: "Full-stack Developer with practical experience in building responsive web applications using Next.js and Tailwind CSS.",
    quote: "\"Skilled in API integration, scalable UI architecture, and delivering user-centric software solutions.\"",
    image: profileImg,
    primaryButton: "View Portfolio",
    secondaryButton: "Get In Touch"
  },
  about: {
    sectionTitle: "Profile & Education",
    title: "Academic Precision <br /> Meets Practical Development",
    university: "Khon Kaen University",
    degree: "Bachelor of Science in Geo-Informatics (Computer Science), College of Computing",
    paragraphs: [
      "Full-stack Developer with practical experience in building responsive web applications using Next.js and Tailwind CSS. Skilled in API integration, scalable UI architecture, and delivering user-centric software solutions.",
      "My background in Geo-Informatics provides a unique perspective on spatial data management and complex system architecture, allowing me to bridge the gap between technical GIS requirements and modern web standards."
    ],
    stats: [
      { value: "2", label: "Professional Internships" },
      { value: "10+", label: "Technical Tools" }
    ]
  },
  skills: {
    sectionTitle: "Technical Skills",
    title: "The Toolkit",
    categories: [
      {
        id: "languages",
        title: "Languages",
        items: ["TypeScript", "Dart", "Python", "JavaScript", "Go", "SQL", "HTML/CSS"]
      },
      {
        id: "frameworks",
        title: "Frameworks",
        items: ["NestJS", "Next.js", "React", "Flutter", "Tailwind CSS"]
      },
      {
        id: "database",
        title: "DB & ORM",
        items: ["PostgreSQL", "PostGIS", "Prisma"]
      },
      {
        id: "tools",
        title: "Tools",
        items: ["Docker", "Git", "Postman", "Jira"]
      },
      {
        id: "gis",
        title: "GIS & Others",
        items: ["QGIS", "ArcMap", "Figma", "Storybook", "Antigravity"]
      }
    ]
  },
  experience: {
    sectionTitle: "Experience",
    title: "Career Progression",
    items: [
      {
        date: "DEC 2025 — MAR 2026",
        role: "Full-stack Developer Intern",
        company: "Arctic Carbon Tech",
        achievements: [
          "Performed black-box API testing via Postman for data mapping.",
          "Built responsive, scalable UIs with Next.js, Tailwind CSS, and Storybook using design tokens.",
          "Implemented in-app camera restriction for the Azolla App to prevent retrospective uploads.",
          "Conducted Pre-UAT for ERP modules and created technical user manuals guide."
        ]
      },
      {
        date: "APRIL — MAY 2025",
        role: "Backend Developer Intern",
        company: "One Geo Survey",
        achievements: [
          "Designed and implemented relational database schemas using Navicat.",
          "Developed efficient RESTful APIs using Go (Golang) to handle business logic.",
          "Conducted API testing via Postman to ensure pre-deployment stability.",
          "Performed Code Review & Project Management via Git Version Control in GitLab."
        ]
      }
    ]
  },
  projects: {
    sectionTitle: "Featured Work",
    title: "Technical Portfolio",
    description: "A selection of projects that demonstrate expertise in full-stack architecture, API design, and modern frontend technologies.",
    items: [
      {
        title: "Geospatial API Services",
        tags: "GO • POSTGRESQL • GITLAB",
        description: "Backend infrastructure for Surveying data, featuring complex SQL queries and robust Go handlers developed during One Geo Survey internship.",
        image: "https://picsum.photos/seed/geospatial/800/500",
        link: "#"
      },
      {
        title: "Azolla App UI System",
        tags: "NEXT.JS • TAILWIND • STORYBOOK",
        description: "A design-token based UI library and ERP module frontend built during Arctic Carbon Tech internship with high focus on responsiveness.",
        image: "https://picsum.photos/seed/ui-system/800/500",
        link: "#"
      }
    ]
  },
  contact: {
    sectionTitle: "Get in Touch",
    title: "Let's build something <span class=\"bg-gradient-to-r from-[#031f41] to-[#2b6485] bg-clip-text text-transparent\">remarkable.</span>",
    description: "Currently open for junior developer roles or interesting project collaborations.",
    phone: "064-992-9715",
    email: "akeburut.p@kkumail.com",
    location: "252/16 Charan Sanit Wong Rd, Ban Chang Lo, Bangkok Noi, Bangkok 10700"
  },
  footer: {
    name: "Akeburut P.",
    copyright: "© 2024 Built with Technical Precision.",
    socials: {
      github: "#",
      linkedin: "#"
    }
  }
};
